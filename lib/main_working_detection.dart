...
class _DetectorHomePageState extends State<DetectorHomePage> {
  ...
  static const double nmsThreshold = 0.50;
  ...
  Future<void> runDetection(File file) async {
    ...
    try {
      ...
      setState(() {
        detections = filtered;
        detectionText =
            "Aircraft detected: ${filtered.length} (threshold $confidenceThreshold)";
      });
    } catch (e) {
      setState(() {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final box in detections) {
      ...
      final textSpan = TextSpan(
        text: box.confidence.toStringAsFixed(2),
        style: textStyle,
      );
      ...
      textPainter.paint(
        canvas,
        Offset(rect.left, math.max(0, rect.top - 14)),
      );
    }
  }
